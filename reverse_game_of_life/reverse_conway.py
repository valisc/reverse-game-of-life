...
class ConwayBoard:
    def __init__(self,rows=0,cols=0,board=None):
        ...
        if board is not None:
            ...
            self.num_rows = rows
            self.num_cols = cols
            self.board = [ [DEAD]*self.num_cols for i in range(self.num_rows)]
    ...
    def advance(self):
        ...
        new_board = [ [self.__conway_rule(self.board[row][col],self.__count_live_neighbors(row,col)) for col in range(self.num_cols)] for row in range(self.num_rows)]
